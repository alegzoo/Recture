    ...
    <v-dialog :model-value="modelValue" @update:model-value="val => emit('update:modelValue', val)" width="550" scroll-strategy="block" :persistent="quizBeingCreated">
        <v-card>
            <template v-slot:title>
                <h2 class="pt-3 pl-1">NEW QUIZ</h2>
            </template>
...
<script lang="ts" setup>
    ...
    function initialize() {
        ...
        RectureApi.getSubjects().then(result => {
            if (result.success && result.data != null) subjects.value = result.data;
            else closeDialog();
        })
        .catch(reason => {
            closeDialog();
    ...
    function submitHandler() {
        if (validateTitle(title.value) !== true || validateSelect(selectedSubject.value) !== true || selectedSubject.value == null) return;

        quizBeingCreated.value = true;

        RectureApi.createQuiz(selectedSubject.value.subjectId, title.value).then(result => {
            if (result.success) {
                emit("dataModified");
                closeDialog();
            } else {
                errorDialogMessage.value = "Failed to create quiz.";
