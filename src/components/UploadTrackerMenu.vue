<template>
    ...
    >
        <v-card min-width="400">
            ...
            <v-list v-if="uploadStore.trackedUploads.length > 0" class="py-0">
                <UploadTrackerItem v-for="upload in uploadStore.trackedUploads" :upload="upload" @cancel-button-click="() => onItemCancelButtonClicked(upload)"/>
            </v-list>
            <v-list v-else>
    ...
    <ConfirmationDialog v-model="cancelUploadDialogVisible" title="ABORT UPLOAD" :message="'Are you sure you want to stop uploading &quot;'+selectedUpload?.title+'&quot;?'" @option-selected="onConfirmationDialogOptionSelected"/>
</template>
