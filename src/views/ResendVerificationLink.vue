        ...
        <v-row class="h-100">
            ...
            <v-col cols="10" md="6" lg="4" align="center" align-self="center">
                <v-form validate-on="input" @submit.prevent="submitHandler">
                    <h1 class="my-10">Resend verification link</h1>
                    ...
                    <v-row class="mt-8" no-gutters>
                        ...
                        <v-col align="right" class="py-0">
                            <v-btn variant="outlined" type="submit" :disabled="submitDisabled" :loading="submitDisabled" theme="none">Resend link</v-btn>
                        </v-col>
                    </v-row>
                </v-form>
            </v-col>
...
<script lang="ts" setup>
    ...
    function submitHandler() {
        ...
        RectureApi.resendVerificationLink(email.value).then(result => {
            if (result.success) {
                requestSuccessful.value = true;

                messageDialogTitle.value = "SUCCESS";
                messageDialogText.value = "A verification link has been sent to your email!";
                ...
                messageDialogTitle.value = "ERROR";
                messageDialogText.value = "This email is already verified.";
                messageDialogVisible.value = true;
            } else {
                messageDialogTitle.value = "ERROR";
                messageDialogText.value = "Failed to resend verification link.";
                messageDialogVisible.value = true;
            }
        }).catch(reason => {
            messageDialogTitle.value = "ERROR";
