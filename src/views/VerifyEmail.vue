<template>
    <div class="w-100 h-100 d-flex flex-column align-center justify-center">
        <template v-if="state === 'verifying'">
            ...
            <h2 class="flex-grow-0 pt-5">Verifying email</h2>
        </template>
        <template v-else>
            <h2 class="flex-grow-0 pb-5">{{ message }}</h2>
            
            <v-btn v-if="state === 'success'" variant="outlined" class="mb-5" theme="none" to="/signin">Sign in</v-btn>
            <router-link v-else to="/signin">Back</router-link>
        </template>
    </div>  
</template>
...
<script lang="ts" setup>
    ...
    if (!(route.params.code instanceof String || typeof route.params.code === "string")) {
        ...
        RectureApi.verifyEmail(code).then(result => {
            if (result.success) {
                message.value = "Your email has been successfully verified! You may now sign in.";
                state.value = "success";
            } else {
                message.value = "This verification link is invalid or expired.";
                state.value = "error";
            }
        }).catch(reason => {
            message.value = "Failed to verify your email.";
            state.value = "error";
        });
    }
</script>